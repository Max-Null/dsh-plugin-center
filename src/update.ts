...
export function clientBundleUsesRemote(content: string): boolean {
  return /ctx\.remote\.[a-zA-Z_$]+\./.test(content)
}

/** 目标版本客户端 bundle 缓存:name@version → true/false。 */
const remoteUseCache = new Map<string, boolean>()

/** 下载目标 tgz 并抽取 client bundle,判定 remote 服务依赖。
 *  仅返回 boolean 不用 pnpm(直接 registry 下载 tgz + bsdtar 抽文件)。 */
export async function targetClientUsesRemote(name: string, version: string): Promise<boolean> {
  ...
  } catch { /* 任何失败保持 false(不误伤) */ } finally {
...
const npmRepoCache = new Map<string, { at: number, repo: string | null }>()
const NPM_REPO_TTL = 24 * 3600_000

/** 测试用:清空 repository 缓存(生产无调用)。 */
export function clearNpmRepoCache(): void { npmRepoCache.clear() }

/** 读 npm 包根级 repository.url(带 24h 缓存;失败/缺失 null)。 */
export async function npmRepository(packageName: string): Promise<string | null> {
  const hit = npmRepoCache.get(packageName)
  if (hit !== undefined && Date.now() - hit.at < NPM_REPO_TTL) return hit.repo
  let repo: string | null = null
  for (const registry of ['https://registry.npmjs.org', 'https://registry.npmmirror.com']) {
    try {
      const res = await fetch(`${registry}/${packageName}`, { signal: AbortSignal.timeout(8000) })
      if (res.ok) {
        const doc = await res.json() as { repository?: { url?: string } | string }
...
export async function isSameUpstream(localRepoUrl: string | null, packageName: string): Promise<boolean | null> {
  if (localRepoUrl === null) return null
  const npmRepo = await npmRepository(packageName)
  if (npmRepo === null) return null
...
const npmTimeCache = new Map<string, { at: number; published: string | null; readme: string | null }>()
const NPM_TIME_TTL = 5 * 60_000

/** Registry 元数据（UTC 发布时间 + 根 readme）；不可达/无该版本 → null（5min 缓存）。 */
async function npmRegistryInfo(name: string, version: string): Promise<{ published: string | null; readme: string | null }> {
  const key = `${name}@${version}`
  const hit = npmTimeCache.get(key)
  if (hit !== undefined && Date.now() - hit.at < NPM_TIME_TTL) return { published: hit.published, readme: hit.readme }
  const info: { published: string | null; readme: string | null } = { published: null, readme: null }
  for (const registry of ['https://registry.npmjs.org', 'https://registry.npmmirror.com']) {
    try {
      const res = await fetch(`${registry}/${name}`, { signal: AbortSignal.timeout(8000) })
      if (res.ok) {
        const doc = await res.json() as { time?: Record<string, string>; readme?: string }
        const t = doc.time?.[version]
        if (typeof t === 'string' && t !== '') info.published = t
        if (typeof doc.readme === 'string' && doc.readme !== '') info.readme = doc.readme
        break
      }
    } catch { /* next registry */ }
  }
...
export function readmePreviewOf(readme: string): string[] | null {
  const lines: string[] = []
...
export async function detectUpdate(
  ...
  if (changelogSource === 'none') {
    ...
    if (changelog.length > 0) {
      ...
      if (info.published !== null) changelog.push(`v${latest} 发布于 ${info.published.slice(0, 10)}`)
