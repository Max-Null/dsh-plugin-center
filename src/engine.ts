...
export class PluginCenterEngine extends Service {
  ...
  async listInstalled(): Promise<InstalledPlugin[]> {
    ...
    const byBase = new Map<string, { cats: string[]; owners: Set<string>; name: string }>()
    for (const m of merged) {
      ...
      if (cur === undefined) byBase.set(base, { cats: m.categories, owners: new Set([owner]), name: m.name })
      else {
    ...
    const categoriesOf = (p: InstalledPlugin): { cats: string[]; marketName: string | null } => {
      ...
      if (b !== undefined && b.owners.size === 1) return { cats: b.cats, marketName: b.name }
  ...
  async checkUpdates(sinceIso: string): Promise<UpdateDigest[]> {
    ...
    const unique: UpdateDigest[] = []
    for (const d of digests) {
      if (d === null || seen.has(d.name)) continue
      seen.add(d.name)
      unique.push(d)
    }
